...
bucla:
      ...
      case SHARP : if( (c=lexem()) != CON ) {
		   ...
		   linie=liniet;
		   return( lexem() );
      case INSERT: ccc=c=getchar();  	/* ignora	*/
		   if(ccc == '\n') ccc=' ';
		   printf("<%c>",ccc);
		   goto bucla;
      case NEWLN : linie++;
      ...
      case MINUS : return( urmator(c,
				   urmator('>', MINUS, ARROW),
				   DECBEF) );
      case ASSIGN: c=carurm();
		   cascuns=0;
		   if( c == '=' ) return(EQUAL);
		   if( c == ' ' ) return(ASSIGN);
		   if( c == '<' || c == '>' ) {
...
{  register int c, cc;
   register char *pcc;
   char creal;
   valc=0;  pcc=&valc;  cc=0;
   while( (c=mapcar('\'')) >= 0 )
      if(cc++ < NCPC)  *pcc++ = c;
   if(cc>NCPC)  eroare("Constanta caracter lunga",NULL);
   if(cc==1) {
      creal=valc;  valc=creal;
   }
   return(CON);
}

int mapcar(int ac)
{  	register int ccc,a, c, n;
	static mascuns;
	c=ac;
