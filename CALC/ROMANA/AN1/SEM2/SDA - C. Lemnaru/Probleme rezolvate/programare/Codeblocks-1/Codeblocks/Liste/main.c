...
void cautare_noduri(NOD *p_nod,int *key,NOD **q1,NOD **q)
{
    if(p_nod==0)
    {
        printf("\nLista vida");
    ...
    *q=p_nod->urm;
    do
    {
        if((*q)->cheie==key) return;
        *q1=*q; *q=(*q)->urm;
    }while(*q!=p_nod->urm);
    *q=0;
    }

void stergere_nod_cheie_data(NOD **p_nod,int *key)
{

    NOD *q1,*q;
    cautare_noduri(*p_nod,key,&q1,&q);
    if(q==0)
    {
        printf("\nLista nu contine nod de cheia data");
        return 0;
    }
    if(q1==q)
    {
...
int main()
{
    ...
    scanf("%d",&cheie1);
    stergere_nod_cheie_data(&prim,cheie1);
    p=prim;
    if(p!=0)
    do{
        printf(" %d ",p->cheie);
