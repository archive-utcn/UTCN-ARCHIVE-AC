...
ENTITY COMANDA IS 
	...
	OK_CARD,OK_PIN,S,RN,CHITANTA,SOUT,PIN_INCORECT,ELIB_CARD:OUT STD_LOGIC);
END COMANDA;
...
signal SOLD_NOU:std_logic_vector(15 downto 0);
signal WE:std_logic;--write enable pentru sume
...
begin
	P1: IDEN_CARD port map (ADRESA_CARD,BUT,ENABLE,Reset,Addr);
	P2: CITIRE_ADRESA port map (SWITCH,BUT,ADRESA_CARD);--citirea adreselor
	P3: COMP_INTEGER port map (conv_integer(SWITCH),Addr,BUT,CARD_OK);--compararea adreselor
	P4: PIN port map (ADRESA_CARD,PIN_MEM);
	P5: CITIRE_PIN port map (SWITCH,CLK,BUT,PIN_IN);
	P6: COMP_EGAL port map (PIN_MEM,PIN_IN,BUT,OPIN);
	P7: CITIRE_SUMA port map (SWITCH,CLK,BUT,SUMA_IN);
	P8: DMUX port map (OPT,RESET,CLK,ENABLE,Retragere);--Retragere numerar
	P9: DMUX port map (OPT,RESET,CLK,ENABLE,AFIS_SOLD);--Afisare sold
  P10: COMP_MIC port map ("0000001111101000",SUMA_IN,BUT,IES_SUMA); --Compara cu 1000
  P11: COMP_MIC port map (SOLD,SUMA_IN,BUT,IES_SUMA2);--Compara cu soldul
  P12: RAM port map (ADRESA_CARD,WE,SOLD,SOLD_NOU);
  P13: SUBTRACTOR port map (SOLD,SUMA_IN,BUT,SOLD_NOU);
  P14: RECEIPT port map (ENABLE,CLK,OPT,IESIRE_CHIT,FINISH);
  P15: x7seg port map (SOLD(15 downto 12),BUT,AF4);
  P16: x7seg port map (SOLD(11 downto 8),BUT,AF3);
  P17: x7seg port map (SOLD(7 downto 4),BUT,AF2);
  P18: x7seg port map (SOLD(3 downto 0),BUT,AF1);
  P19: DIVIZOR port map (BUT,CLOCK_DIV,CLOCK_AFISOR);
	...
	ELIB_CARD<=FINISH when FINISH='1';
end ARCH_COMANDA;
