...
inidisp()
{  register struct tabcc *p;
   register char *ps;
   register i;
   disputil=0;
   for( p=tabcc; ps=p->numecc; p++ )  {
	i=0;  disputil++;
	while( *ps )  i += *ps++;
...
{  int idisp;
   ...
   if( p->semd&FKEYW )
      if( cautcc() )  return(KEYW);     /* s-a gasit cuvant cheie */
   while( *(pn = p->nume) ) {
      for( pl=tamplex; pl<tamplex+NCPS; )
	if( *pn++ != *pl++ )  goto nu;  // coliziune
      lexc = p;
      return(NAME); /* s-a gasit simbolul fiind introdus deja */
nu:   if( ++p >= &tabdisp[DDISP] )  p = tabdisp;
   }
   if( ++disputil >= DDISP ) { /* intotdeauna ramane o intrare */
			       /* libera in tabsimb            */
      eroare("Depasire tabel de dispersie",NULL);
      exit(1);
   }
   pl=tamplex; /* sa introduca numele nou in prima intrare libera   */
