...
sentence:	nphrase VERB termpunct	{ printf ("Simple noun­verb sentence.\n");
	...
	|	END	{ printf ("Got EOF from lex.\n");
			YYACCEPT;}
/* All these YYACCEPTS are needed so yyparse will return
immediately,
rather than waiting for the first token of the next sentence.
They would not be necessary if the main program were only calling yypase() once*/
